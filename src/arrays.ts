...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map(number=> isNaN(Number(number))? 0 : Number(number));
}
...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map(amount => {const cleaned = amount.replace("$", "");
...
export function injectPositive(values: number[]): number[] {
    ...
    const sumBeforeNegative = values.slice(0,firstNegativeIndex).reduce((sum, value)=> sum + value,0);
    const result = [...values];
